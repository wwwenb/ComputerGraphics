...
void Mesh_Painter::update_texture()
{
	...
	for (unsigned int i = 0; i < this->m_my_meshes_.size(); i++)
	{
		if (this->m_my_meshes_[i]->has_texture == 1) {
			GLuint textures;
...
void Mesh_Painter::update_vertex_buffer()
{
	...
	for (unsigned int m_i = 0; m_i < this->m_my_meshes_.size(); m_i++)
	{
		...
		if (this->m_my_meshes_[m_i]->has_texture== 1) {
		...
		GLuint ModelViewID = glGetUniformLocation(this->program_all[m_i], "ModelView");
		glUniformMatrix4fv(ModelViewID, 1, GL_TRUE, this->m_my_meshes_[m_i]->ModelView);

		GLuint viewMatrixID = glGetUniformLocation(this->program_all[m_i], "viewMatrix");
		glUniformMatrix4fv(viewMatrixID, 1, GL_TRUE, this->m_my_meshes_[m_i]->viewMatrix);

		GLuint hasTextureID = glGetUniformLocation(this->program_all[m_i], "has_texture");
		glUniform1i(hasTextureID, this->m_my_meshes_[m_i]->has_texture);
